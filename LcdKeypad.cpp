...
// ----------------------------------------------------------------------------------------------------
/// fills the dest buff from str, right-padding the dest buffer with the given char.
...
void buttonHandlerCycle()
{
  if (millis() - buttonSampleTime >= 20)
  {
    ...
    for (int i = 0; i < 5; i++) //each possible button
    {
      if (!buttonIdentified && analogReading <= buttonValues[i]) //this relies on the defined nalog values being LARGER than the actual reading received
      {
        btnStateNow = 1;
        buttonIdentified = 1;
        Serial.print("Button identified: index ");
        Serial.print(i);
